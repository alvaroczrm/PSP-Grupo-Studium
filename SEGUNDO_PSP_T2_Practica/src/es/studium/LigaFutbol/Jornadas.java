...
static public ArrayList<int[][]>Temporada(){
...
static public ArrayList<Partidos> ListaDePartidos(int nJornada){
	ArrayList<Partidos> Plista = new ArrayList<Partidos>();
	Partidos partido = null;
	ArrayList<int [][]> JornadaLista= Temporada();
	if (nJornada <=18) {
		for(int i = 0 ; i <10;i++) {
			partido = new Partidos(Equipo.ListaEquipo().get(JornadaLista.get(nJornada)[i][0]),Equipo.ListaEquipo().get(JornadaLista.get(nJornada)[i][1]));
			Plista.add(partido);
		}

	}
	else {
		nJornada = nJornada -19;
		for(int i = 0 ; i<10;i++) {
			partido = new Partidos(Equipo.ListaEquipo().get(JornadaLista.get(nJornada)[i][1]),Equipo.ListaEquipo().get(JornadaLista.get(nJornada)[i][0]));
			Plista.add(partido);
		}
	}
	return Plista;
